...
# Which WiRoc variant is this image? Written by customize-image-host.sh into the chroot /tmp.
WIROC_VARIANT="$(cat /tmp/wiroc-variant 2>/dev/null || echo esp32)"
...
Main() {
	...
	echo "replace the firstlogin script"
	...
	cp /tmp/overlay/sun8i-h3-spi-enable.dtbo /boot/dtb/overlay/
	cp /tmp/overlay/sun8i-h3-bt-uart3.dtbo /boot/dtb/overlay/

	echo "Make ath9k_htc driver always create interface named mesh0"
	cp /tmp/overlay/10-mesh.link /etc/systemd/network/

	echo "Make network manager ignore mesh0"
	cp /tmp/overlay/unmanaged.conf /etc/NetworkManager/conf.d/

	echo "Change overlays"
	if [ "$WIROC_VARIANT" = "nanopiair" ]; then
		# On-board BCM43438 BT: enable UART3 + bluetooth via the bt-uart3 overlay; no SPI/ESP32.
		sed -i -E "s/(overlays=).*/overlays=uart1 uart2 usbhost1 usbhost2 usbhost3 i2c0 bt-uart3/" /boot/armbianEnv.txt
	else
		# ESP32-SPI variant: enable SPI + esp32_spi module; no on-board BT.
		sed -i -E "s/(overlays=).*/overlays=uart1 uart2 uart3 usbhost1 usbhost2 usbhost3 i2c0 spi-enable/" /boot/armbianEnv.txt
		echo "esp32_spi" >> /etc/modules-load.d/modules.conf
	fi
